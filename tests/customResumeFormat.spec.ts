...
test.describe('resumeFromJSONResume', () => {
    ...
    test('should include personal site when url is present', () => {
        ...
        const resume = resumeFromJSONResume(jsonResume);
        const personalSite = resume.profiles.children?.find(p => p.title === 'Personal Site');
    ...
    test('should properly convert work experience', () => {
        ...
        expect(resume.workExperience.children?.[0].title).toBe('Tech Corp');
        expect(resume.workExperience.children?.[0].children).toHaveLength(1);
        expect(resume.workExperience.children?.[0].children?.[0].title).toBe('Software Engineer');
    });
});
